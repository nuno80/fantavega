...
export async function POST() {
  try {
    ...
    console.log("[PROCESS_RESPONSE_TIMERS] Starting processing of expired response timers...");

    const result = await processExpiredResponseTimers();

    console.log(`[PROCESS_RESPONSE_TIMERS] Completed. Processed: ${result.processedCount}, Errors: ${result.errors.length}`);
