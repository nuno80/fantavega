...
interface InvolvedAuction { auction_id: number; player_id: number; player_name: string; current_highest_bidder_id: string | null; current_highest_bid_amount: number; response_deadline: number | null; activated_at: number | null; cooldown_ends_at: number | null; }
interface PlayerWithAssignment { id: number; name: string; role: string; team: string; assignment_price: number; user_id: string; }
interface Manager { user_id: string; manager_team_name: string; current_budget: number; locked_credits: number; total_budget: number; }
interface ManagerWithRoster extends Manager { players: PlayerWithAssignment[]; }
interface UserState { auction_id: number; player_id: number; player_name: string; current_bid: number; user_state: string; response_deadline: number | null; time_remaining: number | null; is_highest_bidder: boolean; }
interface BudgetData { current_budget: number; locked_credits: number; team_name: string; total_budget: number; }
interface LeagueSlots { slots_P: number; slots_D: number; slots_C: number; slots_A: number; }
...
async function getManagersDataLogic(leagueId: string) {
  ...
  const [leagueInfo, managersResult, activeAuctionsResult, autoBidsResult, allPlayersResult] = await Promise.all([
    db.execute({ sql: "SELECT slots_P, slots_D, slots_C, slots_A FROM auction_leagues WHERE id = ?", args: [id] }),
    db.execute({ sql: "SELECT lp.user_id, lp.manager_team_name, lp.current_budget, lp.locked_credits, al.initial_budget_per_manager as total_budget FROM league_participants lp JOIN auction_leagues al ON lp.league_id = al.id WHERE lp.league_id = ? ORDER BY lp.manager_team_name ASC, lp.user_id ASC", args: [id] }),
    db.execute({ sql: "SELECT a.player_id, p.name as player_name, p.role as player_role, p.team as player_team, a.current_highest_bidder_id, a.current_highest_bid_amount, a.scheduled_end_time FROM auctions a JOIN players p ON a.player_id = p.id WHERE a.auction_league_id = ? AND a.status = 'active'", args: [id] }),
    db.execute({ sql: "SELECT a.player_id, COUNT(ab.user_id) as auto_bid_count FROM auto_bids ab JOIN auctions a ON ab.auction_id = a.id WHERE a.auction_league_id = ? AND a.status = 'active' AND ab.is_active = 1 GROUP BY a.player_id", args: [id] }),
    db.execute({ sql: "SELECT p.id, p.name, p.role, p.team, pa.purchase_price as assignment_price, pa.user_id FROM player_assignments pa JOIN players p ON pa.player_id = p.id WHERE pa.auction_league_id = ? ORDER BY p.role, p.name", args: [id] }),
  ]);
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ "league-id": string }> }) {
  ...
  } catch (error) { console.error("[AUCTION-REALTIME] Unexpected error:", error); return NextResponse.json({ success: false, error: "Errore interno del server", timestamp: Math.floor(Date.now() / 1000) }, { status: 500 }); }
}
