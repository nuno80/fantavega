  ...
  try {
    ...
    if (typeof max_amount !== "number" || max_amount < 0) {
      return NextResponse.json(
        { error: "Importo massimo non valido" },
        { status: 400 }
      );
    }
    ...
    if (!participant) {
      return NextResponse.json(
        { error: "Non sei un partecipante di questa lega" },
        { status: 403 }
      );
    }
    ...
    const auction = db
      .prepare(
        "SELECT id, current_highest_bid_amount, status FROM auctions WHERE auction_league_id = ? AND player_id = ? AND status = 'active'"
      )
    ...
    if (!auction) {
      return NextResponse.json(
        { error: "Asta non trovata o non attiva" },
        { status: 404 }
      );
    }
    ...
    const transactionResult = db.transaction(() => {
      ...
      if (newMaxAmount > 0) {
        ...
        if (newMaxAmount <= auction.current_highest_bid_amount) {
          throw new Error(
            `Il prezzo massimo deve essere superiore all'offerta attuale (${auction.current_highest_bid_amount})`
          );
        }
        db.prepare(
          `
        ...
        db.prepare(
          "UPDATE auto_bids SET is_active = FALSE, updated_at = strftime('%s', 'now') WHERE auction_id = ? AND user_id = ?"
      ...
      return {
        message:
          newMaxAmount > 0
            ? "Auto-offerta impostata con successo"
            : "Auto-offerta disattivata",
...
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const autoBid = db
      .prepare(
        "SELECT max_amount, is_active, created_at, updated_at FROM auto_bids WHERE auction_id = ? AND user_id = ?"
      )
