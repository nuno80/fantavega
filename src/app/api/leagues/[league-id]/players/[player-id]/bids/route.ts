// src/app/api/leagues/[league-id]/players/[player-id]/bids/route.ts v.1.4 - Patched with 8dbeada changes
...
export async function POST(request: Request, context: RouteContext) {
  console.log(
    "!!!!!!!!!! POST HANDLER REACHED for /api/leagues/[league-id]/players/[player-id]/bids !!!!!!!!!!_v1.4"
  );
  ...
  try {
    ...
    // The error is re-thrown and will be caught by the outer try-catch in the original implementation
    // but since we are now in a separate function, we should handle it here or rethrow.
    // For simplicity, we'll let the generic error handler below manage it.
...
async function processBidRequest(
  ...
  user: User, // Type from Clerk's currentUser()
  leagueIdNum: number,
  playerIdNum: number
): Promise<NextResponse> {
  try {
    // 2.1. Parsing del body della richiesta
    ...
    // 2.3. Logica di offerta: determina se creare una nuova asta o fare un'offerta su una esistente
    console.log(
    ...
    if (
      ...
      result = await placeBidOnExistingAuction({
        ...
        bidAmount,
        bidType,
        autoBidMaxAmount: body.max_amount,
      });
      httpStatus = 200;
      console.log(
...
export async function GET(_request: Request, context: RouteContext) {
  console.log(
    "!!!!!!!!!! GET HANDLER REACHED for /api/leagues/[league-id]/players/[player-id]/bids !!!!!!!!!!_v1.4"
  );
  try {
    const user = await currentUser();
    if (!user) {
