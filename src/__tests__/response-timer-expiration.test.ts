...
describe("Response Timer Expiration", () => {
  beforeEach(() => {
    ...
    try {
      db.prepare(
        "DELETE FROM user_auction_response_timers WHERE auction_id = 1"
      ).run();
      db.prepare("DELETE FROM auctions WHERE id = 1").run();
      db.prepare(
        "DELETE FROM league_participants WHERE league_id = 1 AND user_id = ?"
      ).run("test-user-1");
  ...
  it("should properly process expired response timers and free up auction slots", async () => {
    ...
    const initialAuction = db
      .prepare("SELECT * FROM auctions WHERE id = 1")
      .get() as Auction;
    expect(initialAuction.status).toBe("active");

    const initialTimer = db
      .prepare("SELECT * FROM user_auction_response_timers WHERE id = 1")
      .get() as ResponseTimer;
    ...
    const playerAssignment = db
      .prepare(
        "SELECT * FROM player_assignments WHERE player_id = 1 AND user_id = ?"
      )
      .get("test-user-1") as PlayerAssignment | undefined;
    ...
    const participant = db
      .prepare(
        "SELECT * FROM league_participants WHERE user_id = ? AND league_id = 1"
      )
      .get("test-user-1") as LeagueParticipant;
  ...
  it("should handle expired timers without a highest bidder", () => {
    ...
    db.prepare(
      "UPDATE auctions SET current_highest_bidder_id = NULL WHERE id = 1"
    ).run();
