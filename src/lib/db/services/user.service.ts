...
export async function getUsersWithLeagueDetails(): Promise<UserWithLeagueDetails[]> {
  const userListResponse = await (await clerkClient()).users.getUserList({ limit: 200, orderBy: "-created_at" });
  if (!userListResponse || !Array.isArray(userListResponse.data)) { return []; }
  const getLeaguesForUserStmt = db.prepare(`SELECT al.id, al.name, al.status FROM auction_leagues al JOIN league_participants lp ON al.id = lp.league_id WHERE lp.user_id = ?`);
  return userListResponse.data.map((user: User) => ({
    id: user.id,
    primaryEmail: user.emailAddresses.find((e: EmailAddress) => e.id === user.primaryEmailAddressId)?.emailAddress,
...
export async function setUserRole(userId: string, role: string): Promise<{ success: boolean; message: string }> {
  if (!userId || !role) { return { success: false, message: "User ID e ruolo sono obbligatori." }; }
  try {
    await (await clerkClient()).users.updateUserMetadata(userId, { publicMetadata: { role: role } });
    return { success: true, message: "Ruolo aggiornato con successo." };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : "Errore sconosciuto.";
    return { success: false, message: `Aggiornamento fallito: ${errorMessage}` };
  }
}

// 5. Funzione CORRETTA per trovare utenti idonei
export async function getEligibleUsersForLeague(leagueId: number): Promise<EligibleUser[]> {
  const allUsersResponse = await (await clerkClient()).users.getUserList({ limit: 500 });
  if (!allUsersResponse || !Array.isArray(allUsersResponse.data)) { return []; }

  const allManagers = allUsersResponse.data.filter(user => (user.publicMetadata?.role as string || 'manager') === 'manager');
  
  const participantsStmt = db.prepare(`SELECT user_id FROM league_participants WHERE league_id = ?`);
