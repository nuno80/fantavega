...
try {
  ...
  console.log(`Checking auctions and bids for league ${leagueId}`);
  ...
  if (activeAuctions.length > 0) {
    console.log('\nChecking bids for active auctions:');
  ...
  // Check user auction response timers
  console.log('\nChecking user auction response timers:');
  const timersStmt = db.prepare(`
    ...
    WHERE urt.auction_id IN (SELECT id FROM auctions WHERE auction_league_id = ?)
    ORDER BY urt.auction_id, urt.user_id
  `);
  
  const timers = timersStmt.all(leagueId);
